...
public:
  ...
  // Constructor that sets the seed from file with different primes
  Random(const char* PrimesFile, const char* SeedFile, unsigned int n);
  // Destructor
  ~Random();
  ...
  // Method to set a different seed and primes for the RNG taking them from files
  void SetRandFromFile( const char* primes, const char* seed, int unsigned n );
  // Method to save the seed to a file
  void SaveSeed();
  // Method to save the seed to a file when using parallel computing
  void SaveSeed(int rank);
  ...
  arma::vec Sphere( int dimentions, double radius );
};
