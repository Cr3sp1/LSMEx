...
// Reads the American capitals
mat Capitals( int ncities ){
    // Open the input file
    std::ifstream inputFile("American_capitals.in");
    ...
    mat result(ncities, 2);
    // Read the data from the file
    for (int i = 0; i < ncities; ++i) {
        inputFile >> name >> description >> result(i, 0) >> result(i, 1);
    }
...
vec ReceiveVec( int vsize, int fromRank, int tag, MPI_Comm comm ){
    vec v(vsize);
    //cout << "Receiving" + to_string(tag) << endl;
    MPI_Recv(v.memptr(), vsize, MPI_DOUBLE, fromRank, tag, comm, MPI_STATUS_IGNORE);
    //cout << "Received" + to_string(tag) << endl;

    return v;
}

// Sends a matrix using MPI
void SendMat( const mat &m, int toRank, int tag, MPI_Comm comm ){
    int mcol = m.n_cols;
    int mrow = m.n_rows;
    // cout << "Sent" + to_string(tag) << endl;
    MPI_Send(m.memptr(), mcol*mrow, MPI_DOUBLE, toRank, tag, comm);
}

// Receives a matrix using MPI
mat ReceiveMat( int nrows, int ncols, int fromRank, int tag, MPI_Comm comm ){
    mat m(nrows, ncols);
    // cout << "Receiving" + to_string(tag) << endl;
    MPI_Recv(m.memptr(), nrows*ncols, MPI_DOUBLE, fromRank, tag, comm, MPI_STATUS_IGNORE);
    // cout << "Received" + to_string(tag) << endl;
    
    return m;
}

// Exchanges the best individuals
void Switcherout( mat &Pop, const mat &dist, int rank, int size, Random &shared ) {
    
    int psize = Pop.n_rows;

    // We recycle the function that generates paths to generate a random order of switching, node S(i) sends to S(i+1), S(size-1) sends to S(0)
    vec S = Path( size+1, shared );

    // if(rank == 0) {
    //     cout << "Send path: " << S.t() << endl;
    // }
    // if(rank == 1) {
    //     cout << "Check path: " << S.t() << endl;
    // }

    // Sorts the population best to worst
    LSort( Pop, dist );

    // Node S(size-1) starts it
    if( rank == S(size-1) ){
        // Sends and receives the best path, and replaces the worst path with the one received
        SendVec( Pop.col(0), S(0), S(size-1), MPI_COMM_WORLD );
        Pop.col(Pop.n_cols-1) = ReceiveVec( psize, S(size-2), S(size-2), MPI_COMM_WORLD );
    }

    // Receives and sends for node S(0)
    if( rank == S(0) ){
        Pop.col(Pop.n_cols-1) = ReceiveVec( psize, S(size-1), S(size-1), MPI_COMM_WORLD );
        SendVec( Pop.col(0), S(1), S(0), MPI_COMM_WORLD );
    }

    // Receives and sends for the other nodes
    for( int i = 1; i < size-1; i++ ){
        if( rank == S(i) ){
            Pop.col(Pop.n_cols-1) = ReceiveVec( psize, S(i-1), S(i-1), MPI_COMM_WORLD );
            SendVec( Pop.col(0), S(i+1), S(i), MPI_COMM_WORLD );
        }
    }

}

mat TotalPop(const mat& Pop, int rootRank, int rank, int size) {
    int ncol = Pop.n_cols;
    int nrow = Pop.n_rows;
    int totalCols = ncol * size;

    // Gather matrices from all processes
    mat gatheredMatrix;
    if (rank == rootRank) {
        gatheredMatrix.set_size(nrow, totalCols);
    }
    MPI_Gather(Pop.memptr(), ncol * nrow, MPI_DOUBLE, gatheredMatrix.memptr(), ncol * nrow, MPI_DOUBLE, rootRank, MPI_COMM_WORLD);

    if( rank == rootRank) return gatheredMatrix;
    return Pop;
}
