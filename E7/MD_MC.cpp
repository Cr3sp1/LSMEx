...
void Input(void)
{
  ifstream ReadInput, ReadConf, ReadVelocity, Primes, Seed, ChooseInput;  //Added ChooseInput
...
// Choose the Input     //Added
  ...
  if( InCho == 0 ) InName = "solid";
  if( InCho == 1 ) InName = "liquid";
  if( InCho == 2 ) InName = "gas";
...
//Prepare arrays for measurements
  ...
  ie = 3; //Total energy
  iw = 4; //Pressure      //Added
  n_props = 5 + nbins; //Number of observables     //Changed 4->5 + nbins


//Calculates tail corrections    //Added
  vtail = 8*pi*rho*( 1/(9*pow(rcut,9)) - 1/(3*pow(rcut,3)) );
  ptail = 32*pi*rho*rho*( 1/(9*pow(rcut,9)) - 1/(6*pow(rcut,3)) );

  cout << "V correction: " << vtail << endl;
  cout << "P correctiin: " << ptail << endl;
...
//Read initial configuration
  ...
  {
    ReadConf.open("config_"+InName+".in");   //Changed "config.in" -> "config_"+InName+".in"
...
//Added additional equilibration
  for(int i = 0; i < 5000; i++) Move();
...
//Print initial values for measured properties
  ...
  cout << "Initial pressure         = " << walker[iw]<< endl;   //Added
...
{
  double v = 0.0, kin=0.0, w = 0.0;    //Added w
  double vij;
  double dx, dy, dz, dr;

  for(int i = 0; i < n_props; i++) walker[i] = 0;     //Resets the walkers    //Added
...
// distance i-j in pbc
      ...
      if(dr < rcut)
      {
        ...
        //Added to calculate <w>
        w += 1.0/pow(dr,12) - 0.5/pow(dr,6);
      }
      //Added to calculate g_r
      if(dr < box/2)
      {
      bin = dr/bin_size + 5;
      walker[bin]+=2;
  ...
  walker[iw] = rho*walker[it]+ w * 16 / vol;  // Pressure     //Added 
...
void Accumulate(void) //Update block averages
{
   ...
   if(nblk == 1){
    ...
    UN.open("output_"+InName+"_UN.dat",ios::app);
    UN << setw(wd) << blk_norm  << setw(wd) << walker[iv]/(double)npart  + vtail << endl;
   }
}
...
void Averages(int iblk) //Print results for current block
{
   ...
   ofstream Epot, Ekin, Etot, Temp, Pres, GrBlk, GrFin;     // Added Pres, GrBlk, GrFin
   const int wd=12;
    
    cout << "Block number " << iblk << endl;
    cout << "Acceptance rate " << accepted/attempted << endl << endl;
    
    // Changed "output_type.dat" -> "output_"+InName+"_type.dat"
    Epot.open("output_"+InName+"_epot_"+MCMD+".dat",ios::app);
    // Ekin.open("output_"+InName+"_ekin_"+MCMD+".dat",ios::app);
    // Temp.open("output_"+InName+"_temp_"+MCMD+".dat",ios::app);
    Etot.open("output_"+InName+"_etot_"+MCMD+".dat",ios::app);
    Pres.open("output_"+InName+"_pres_"+MCMD+".dat",ios::app);      //Added
    // GrBlk.open("output_"+InName+"_grblock_"+MCMD+".dat",ios::app);       //Added
    GrFin.open("output_"+InName+"_gr_"+MCMD+".dat",ios::app);       //Added
    
    stima_pot = blk_av[iv]/blk_norm/(double)npart  + vtail ; //Potential energy   //Added + vtail
    ...
    stima_etot = blk_av[ie]/blk_norm/(double)npart  + vtail; //Total energy   //Added + vtai
...
//Does everything for g_r     //Added
    for( int i = 5 ; i < nbins+5; i++ ){
      g_norm = rho*npart*4*M_PI*( pow(bin_size*(i-5) + bin_size, 3) - pow(bin_size*(i-5), 3) )/3;
      stima_gdir = blk_av[i]/(blk_norm*g_norm);
      glob_av[i] += stima_gdir;
      glob_av2[i] += stima_gdir*stima_gdir;
      err_gdir=Error(glob_av[i], glob_av2[i], iblk);

      // GrBlk << setw(wd) << iblk << setw(wd) << (i-5)*bin_size <<  setw(wd) << stima_gdir << endl;
      if(iblk == nblk) {
        GrFin << setw(wd) << (i-5)*bin_size <<  setw(wd) << glob_av[i]/((double)nblk) << setw(wd) << err_gdir << endl;
      }
    }
